...
This launcher shares platform-neutral checking logic with
``ucm_model_check_common.py``. It does not load a checkpoint and it does not
...
Run after editing the "User configuration" and "UCM configuration" sections::
    ...
    python examples/model_check/ucm_model_check_ascend.py

The installed vLLM must expose the V1 APIs used by UCM (tested API families:
vLLM 0.18/0.20).  API absence is treated as an unsupported environment rather
than silently falling back to a hand-written cache layout.
"""
...
import torch
from ucm_model_check_common import (
    CacheFixture,
    UnsupportedEnvironment,
    current_vllm_config_context,
    log,
)
from ucm_model_check_common import make_cache as make_common_cache
from ucm_model_check_common import make_config as make_common_config
from ucm_model_check_common import (
    make_worker,
...
# =========================== User configuration ===========================
# Edit only these values. No command-line parameters are supported.
...
# Multiple backends are separated by ':', e.g. "./data0:./data1".
storage_backends = "./build/data"
# Physical NPU card visible to this checker process, e.g. "7".
visible_devices = "0"
...
if __name__ == "__main__":
    if len(sys.argv) != 1:
        raise SystemExit(
            "Edit this file to configure the probe; command-line arguments are disabled."
        )
