...
from ucm.integration.vllm import group_plan
from ucm.integration.vllm.group_hash import (
    GroupInfo,
    KVCacheGroupManager,
    _normalize_tensor_size_list,
...
class UCMHybridLinearAttentionConnector(UCMDirectConnector, SupportsHMA):
    ...
    def get_num_new_matched_tokens(
        ...
        external_hit_tokens, external_hit_lcm_blocks, mamba_prefetch_hashes = (
            self.group_manager.lookup_external_hit_tokens(
                ...
                on_lookup_error=lambda: _record_counter(
    ...
    def _generate_hla_dispatch_meta(
        ...
        state_hash_fn = self.group_manager.compute_mamba_align_state_hash
        if need_load and external_hit_lcm_blocks > 0:
            # Full-attention slices first (for MLA rank-0-only dump), then
            # one mamba state anchor per state group.
            load_slices = group_plan.plan_load(
                groups_by_id,
                lcm_block_size,
                state_hash_fn,
            ...
            load_slices = []
        load_ucm_block_ids = [
            block_id for sl in load_slices for block_id in sl.ucm_block_ids
        ]
        load_vllm_block_ids = [
            block_id for sl in load_slices for block_id in sl.vllm_block_ids
        ]
        load_full_attn_count = (
            sum(len(sl.ucm_block_ids) for sl in load_slices if not sl.is_state)
            if self.is_mla
            else 0
        )
        ...
        if req_meta.token_processed < req_meta.num_token_ids:
            ...
            dump_slices = group_plan.plan_dump(
                groups_by_id,
                lcm_block_size,
                state_hash_fn,
                req_meta.group_ucm_block_ids,
                req_meta.group_vllm_block_ids,
                token_processed=dump_tok_start,
                dump_tok_end=dump_tok_end,
            )
        else:
            dump_slices = []
        dump_ucm_block_ids = [
            block_id for sl in dump_slices for block_id in sl.ucm_block_ids
        ]
        dump_vllm_block_ids = [
            block_id for sl in dump_slices for block_id in sl.vllm_block_ids
        ]
        dump_full_attn_count = (
            sum(len(sl.ucm_block_ids) for sl in dump_slices if not sl.is_state)
            if self.is_mla
            else 0
        )
